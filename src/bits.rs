...
///
/// Panics if `bits > 64`.
/// May panic if `(bit_offset + bits - 1) / 64` is not a valid index in the array.
pub fn write_int<T: IndexMut<usize, Output = u64>>(array: &mut T, bit_offset: usize, value: u64, bits: usize) {

    let value = value & low_set(bits);
    let (index, offset) = split_offset(bit_offset);

    if offset + bits <= WORD_BITS {
        array[index] &= high_set(WORD_BITS - bits - offset) | low_set(offset);
        array[index] |= value << offset;
    } else {
        array[index] &= low_set(offset);
        array[index] |= value << offset;
        array[index + 1] &= high_set(2 * WORD_BITS - bits - offset);
...
///
/// May panic if `(bit_offset + bits - 1) / 64` is not a valid index in the array.
pub fn read_int<T: Index<usize, Output = u64>>(array: &T, bit_offset: usize, bits: usize) -> u64 {
    ...
    if offset + bits <= WORD_BITS {
        ...
        first | ((array[index + 1] & low_set((offset + bits) & OFFSET_MASK)) << (WORD_BITS - offset))
    }
}
